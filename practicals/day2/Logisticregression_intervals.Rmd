...
## Is this the same as Agresti?
Let's find out:
```{r}
#Let's take a simple model, because I don't understand Agresti's notation 
fit1 <- titanic %$% 
...
```{r}
# From package marginaleffects
predictions(fit) %>% head() #interval not okay
predict(fit, se.fit = TRUE) %>%  
  as.data.frame() %>% 
  mutate(prob = plogis(fit), 
         lwr = plogis(fit - qt(.975, Inf) * se.fit),
         upr = plogis(fit + qt(.975, Inf) * se.fit)) %>% head()
get_predicted_ci(fit, predictions = get_predicted(fit), ci_type = "prediction") %>% head()
```


## Are confidence intervals wider than prediction
